...
class SegDataset(Dataset):
    def __init__(self, df, split, sample=None, masking=None):
        self.df = df
        self.split = split
...
def train_seg(model,
    ...
    for epoch in range(epochs):
        ...
        for batch in loop:
            a_img = batch["a_img"].to(device)
            p_img = batch["p_img"].to(device)
            lesion = batch["fused_lesion"].to(device).float()
...
def evaluate_model(model, test_loader, device = None):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # correct += (preds == lesion).sum().item()            # counts how many pixels are correctly predicted and accumulates this number for each batch
            # total += lesion.numel()                              # accumulates the total number of pixels in the ground truth masks for each batch
...
def main():
    ...
    train_dataset = SegDataset(df, split = "train")
    val_dataset = SegDataset(df, split = "val")
    ...
    pred_mask = evaluate_model(trained_model, test_loader)
